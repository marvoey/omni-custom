...
type Props = {
  content: ContentProps<typeof RestaurantEntity>;
};

const RestaurantDetail: React.FC<Props> = ({ content }) => {
  const { pa, src } = getPreviewUtils(content);
  const { getAlt, getSrcset } = damAssets(content);

  const heroImageUrl = content.heroImage ? (src(content.heroImage) || content.heroImage.url?.default) : null;
  const reservationUrl = content.reservationUrl?.default;
  const menuUrl = content.menuLink?.url?.default;
  const menuTitle = content.menuLink?.title;
  ...
  return (
    <article className="bg-[#fdfcf8] min-h-screen font-sans text-stone-900">
      ...
      <section className="relative h-[70vh] w-full">
        {heroImageUrl && (
          <div {...pa('heroImage')} className="absolute inset-0 z-0">
            <Image
              src={heroImageUrl}
              alt={getAlt(content.heroImage, content.restaurantName || '')}
              fill
              priority
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-white">
          <span {...pa('cuisine')} className="uppercase tracking-[0.3em] text-sm mb-4 bg-black/40 px-4 py-1 backdrop-blur-md">
            {content.cuisine}
          </span>
          <h1 {...pa('restaurantName')} className="text-6xl md:text-8xl font-serif font-bold tracking-tight text-center">
        ...
        <div className="lg:col-span-5 bg-stone-100 p-10 self-start border-l-4 border-stone-800">
          ...
          <div className="space-y-8">
            <div>
              ...
              <p {...pa('hours')} className="text-lg whitespace-pre-line">{content.hours}</p>
            </div>
            ...
            <div>
              ...
              <p {...pa('locationTags')} className="text-lg">{content.locationTags?.join(', ')}</p>
            </div>
