...
import type { OfferEntity as OfferEntityContentType } from '@/cms/content-types/OfferEntity';

type Props = {
  content: ContentProps<typeof OfferEntityContentType>;
};

export default function OfferEntityCard({ content }: Props) {
  ...
  const badge = content.badgeLabel ?? '';
  const bookingWindow = content.bookingWindow ?? '';
  const stayWindow = content.stayWindow ?? '';
  const offerCode = content.offerCode ?? '';
  const ctaLabel = content.ctaLabel ?? 'Reserve';
  const ctaUrl =
    (content.ctaUrl as { url?: { default?: string } } | undefined)?.url?.default ?? '#';
  const primaryCategory =
    (content.SingleTaxonomy as { name?: string } | undefined)?.name ?? '';
  ...
  return (
    <div className="max-w-md mx-auto bg-white border border-gray-100 rounded-sm shadow-xl overflow-hidden group transition-all duration-300 hover:-translate-y-1">
      <div className="relative h-60 w-full overflow-hidden">
        ...
        {badge && (
          <div {...pa('badgeLabel')} className="absolute top-4 left-4 bg-[#c5a059] text-white px-3 py-1 text-[10px] font-bold uppercase tracking-widest rounded-sm">
            {badge}
          </div>
        )}
      </div>
      ...
      <div className="p-8 font-sans">
        ...
        <p {...pa('teaser')} className="text-sm leading-relaxed text-gray-600 mb-6 font-light antialiased">
          {teaser}
        </p>
        ...
        {offerCode && (
          <div className="mb-6 text-[10px] uppercase tracking-widest text-gray-500">
            Offer Code: <span className="font-bold text-[#1a1a1a]">{offerCode}</span>
          </div>
        )}
